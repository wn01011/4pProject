...
const fruitCategories = function (category) {
  try {
    ...
    tempEnvA.innerText = `국·탕·찌개`;
    tempOniA.innerText = `두부·어묵·부침개`;
    tempPotaA.innerText = `밀키트·메인요리`;
    tempCucA.innerText = `베이컨·햄·통조림`;
    tempSpiA.innerText = `밑반찬`;
...
axios
  .post("/api/product/category", { data: "국" })
  .then((data) => {
    fruitCategories();
