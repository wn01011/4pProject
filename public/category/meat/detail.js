...
const fruitCategories = function (category) {
  try {
    ...
    tempBroA.innerText = `계란류`;
    tempEnvA.innerText = `국내산 소고기`;
    tempOniA.innerText = `닭·오리고기`;
    tempPotaA.innerText = `수입산 소고기`;
    tempCucA.innerText = `양념육·돈까스`;
    tempSpiA.innerText = `돼지고기`;
    tempFroA.innerText = `양고기`;
...
axios
  .post("/api/product/category", { data: "정육" })
  .then((data) => {
    fruitCategories();
