...
const categoryGun = document.getElementById("item-gun");
...
const fruitCategories = function (category) {
  try {
    ...
    const tempGunA = document.createElement("a");

    tempAllA.innerText = "전체보기";
    tempBroA.innerText = `오징어·낙지·문어`;
    tempBeanA.innerText = `김·미역·해조류`;
    tempEnvA.innerText = `제철수산`;
    tempOniA.innerText = `새우·게·랍스터`;
    tempGunA.innerText = `건어물·다시팩`;
    tempPotaA.innerText = `생선류`;
    tempCucA.innerText = `해산물·조개류`;
    tempSpiA.innerText = `굴비·반건류`;
    tempFroA.innerText = `수산가공품`;
...
axios
  .post("/api/product/category", { data: "수산" })
  .then((data) => {
    fruitCategories();
