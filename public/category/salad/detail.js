...
const fruitCategories = function (category) {
  try {
    ...
    tempBroA.innerText = `떡볶이·튀김·순대`;
    tempBeanA.innerText = `선식·시리얼`;
    tempEnvA.innerText = `샐러드·닭가슴살`;
    tempOniA.innerText = `피자·핫도그·만두`;
    tempPotaA.innerText = `도시락·밥류`;
    tempCucA.innerText = `폭립·떡갈비·안주`;
    tempSpiA.innerText = `파스타·면류`;
    tempFroA.innerText = `죽·스프·카레`;
